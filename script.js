...
function zoomAt(screenX, screenY, newScale) {
  // Calculate the mouse position relative to the world
  const worldX = (screenX - originX) / scale;
  const worldY = (screenY - originY) / scale;

  // Update the target scale (without easing)
...
viewport.addEventListener("wheel", e => {
  ...
  // Calculate the new scale based on zoom factor
  const newScale = Math.min(
    Math.max(targetScale * zoomFactor, MIN_SCALE),
    MAX_SCALE
  );

  // Ensure zoom is focused on the mouse position (screenX, screenY)
...
viewport.addEventListener("touchmove", e => {
  ...
  if (e.touches.length === 2) {
    ...
    // Ensure zoom is focused on the center of the pinch (midX, midY)
