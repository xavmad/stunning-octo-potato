...
function activateGroup(project) {
  ...
  /* zoom out to the center of the whole canvas */
  targetScale = 0.7;
  ...
  setTimeout(() => {
    ...
    // We now zoom out from the center of the canvas (whole viewport)
    const worldCenterX = (window.innerWidth * 0.5 - originX) / scale;
    const worldCenterY = (window.innerHeight * 0.5 - originY) / scale;

    /* LEFT CENTER OF SCREEN — WORLD SPACE */
    const CENTER_X = worldCenterX - (window.innerWidth * 0.35) / scale;  // Left align the group
    const CENTER_Y = worldCenterY;

    /* adaptive radius */
    const MAX_RADIUS =
      Math.min(window.innerWidth * 0.28, window.innerHeight * 0.45);
    ...
    /* ==========================================
       CAMERA FRAMES GROUP — Fixed from canvas center
    ========================================== */

    targetScale = 1;
    // Instead of using previous origin, set camera center to fixed "canvas center"
    targetOriginX = CAMERA_CENTER_X - CENTER_X;  // Adjust to the group center location
    targetOriginY = CAMERA_CENTER_Y - CENTER_Y;  // Adjust to the group center location

    // Smooth zoom-in transition from the canvas center
