...
function activateGroup(project) {
  ...
  setTimeout(() => {
    ...
    /* ==========================================
       LEFT-HALF WORLD LAYOUT
    ========================================== */

    // world-space left half
    const SAFE_LEFT  = -window.innerWidth * 0.5;
    const SAFE_RIGHT = 0;

    // spiral center inside left half
    const CENTER_X = (SAFE_LEFT + SAFE_RIGHT) / 2;
    const CENTER_Y = 0;

    /* max spiral size keeps everything visible */
    const MAX_RADIUS =
      Math.min(
