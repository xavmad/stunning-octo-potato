...
function cameraLoop() {
  ...
  if (Math.abs(targetScale - scale) < 0.0012) scale = targetScale;

if (Math.abs(wheelDeltaBuffer) > 0.01) {

  const step = wheelDeltaBuffer * 0.12;
  wheelDeltaBuffer -= step;

  const zoom = Math.pow(1.0005, -step);
