...
function activateGroup(project) {
  ...
  setTimeout(() => {
    ...
    /* LEFT HALF OF SCREEN — WORLD SPACE */
...
const MAX_RADIUS =
  Math.min(
    ...
    groupImages.forEach((img, i) => {
     ...
     const RING_SPACING = IMAGE_SIZE * 0.85;

      const SPIRAL_STEP = IMAGE_SIZE * 0.9;
      const ANGLE_STEP  = 0.75;

      let angle = i * ANGLE_STEP;
      let radius = angle * SPIRAL_STEP / (Math.PI * 2);
      ...
      let angle = direction * Math.ceil(i / 2) * 1.1;
