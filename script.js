...
function clampPanTargets() {
  ...
  const worldW = window.innerWidth  * WORLD_SCALE * scale;
  const worldH = window.innerHeight * WORLD_SCALE * scale;

  const viewW = window.innerWidth;
  const viewH = window.innerHeight;

  const minX = viewW - worldW - PAN_MARGIN;
  const maxX = PAN_MARGIN;

  const minY = viewH - worldH - PAN_MARGIN;
  const maxY = PAN_MARGIN;

  targetOriginX = Math.min(maxX, Math.max(minX, targetOriginX));
  targetOriginY = Math.min(maxY, Math.max(minY, targetOriginY));
}
