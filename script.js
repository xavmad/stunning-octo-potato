...
if (!isFinite(originX) || !isFinite(originY) || !isFinite(scale)) {
  originX = 0;
  originY = 0;
  scale = 1;
  targetScale = 1;
...
function cameraLoop() {
  ...
  // smooth wheel zoom (desktop)
  if (Math.abs(smoothWheel) > 0.1) {
  ...
  // pan easing — disabled during pinch
  if (!pinchActive) {
  ...
  // settle
  if (Math.abs(targetOriginX - originX) < 0.01)
    originX = targetOriginX;

  if (Math.abs(targetOriginY - originY) < 0.01)
    originY = targetOriginY;

  if (Math.abs(targetScale - scale) < 0.0005)
    scale = targetScale;
...
viewport.addEventListener("touchend", e => {
  ...
  if (e.touches.length < 2 && pinchActive) {
    ...
    // 🔒 lock camera state — prevents bounce
    targetScale = scale;
    targetOriginX = originX;
    targetOriginY = originY;
  }
