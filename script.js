...
function cameraLoop() {
  ...
  // Directly apply the scale (zoom) without easing
  scale = targetScale;

  // Apply the new target origin when close enough
...
function zoomAt(screenX, screenY, newScale) {
  // Get the bounding rect of the viewport to convert mouse position into canvas space
  const rect = viewport.getBoundingClientRect();

  // Convert mouse position into canvas space
  const x = screenX - rect.left;
  const y = screenY - rect.top;

  // Calculate the mouse position in world coordinates (before scaling)
  const worldX = (x - originX) / scale;
  const worldY = (y - originY) / scale;

  targetScale = newScale;  // Set the new scale directly

  // Update the target origin to keep the zoom focused on the mouse position
  targetOriginX = x - worldX * newScale;
  targetOriginY = y - worldY * newScale;

  // Update the positions of all images relative to the new scale
  images.forEach(img => {
    img._x = (img._tx - originX) * newScale;
    img._y = (img._ty - originY) * newScale;
    img.style.transform = `translate(${img._x}px, ${img._y}px) translate(-50%, -50%)`;
  });
}
...
viewport.addEventListener("wheel", e => {
  ...
  // Zoom factor based on the direction of the wheel scroll
...
function floatImages() {
  if (!activeProject || IS_MOBILE) {
    images.forEach(img => {
      ...
      // Adjust roaming space for images during floating
      ...
      // Update image position based on zoom level and camera position
      img.style.transform = `translate(${img._x}px, ${img._y}px) translate(-50%, -50%)`;
    });
  }
