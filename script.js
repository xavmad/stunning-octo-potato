...
function cameraLoop() {
  ...
  // Directly apply scale, no easing
  scale = targetScale;

  // If we're close to the target origin, snap to it
...
function zoomAt(screenX, screenY, newScale) {
  const rect = viewport.getBoundingClientRect(); // Get viewport bounds
  ...
  targetScale = newScale;  // Directly set the new scale

  // Update the origin to keep zoom centered on mouse position
