...
function activateGroup(project) {
  ...
  setTimeout(() => {
    ...
    targetScale = 1;
    // Correct camera framing: adjust the origin based on scale
    targetOriginX = CAMERA_CENTER_X - CENTER_X * scale; // Fixed camera origin calculation with scale
    targetOriginY = CAMERA_CENTER_Y - CENTER_Y * scale; // Same fix for Y-axis
