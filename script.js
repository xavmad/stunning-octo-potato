...
function activateGroup(project) {
  ...
  setTimeout(() => {
    ...
    /* ============================================
       FIXED LEFT-CENTER POSITIONING — SCREEN SPACE
    ============================================ */
    ...
    // Use the exact same spiral math as your original script
    groupImages.forEach((img, i) => {
      img.style.width = IMAGE_SIZE + "px";

      // Golden-angle spiral for the layout (same as original math)
      const angle = i * Math.PI * (3 - Math.sqrt(5)); // Golden angle
      const radius = Math.sqrt(i) * (IMAGE_SIZE + MIN_GAP);

      // **Key Fix**: Position the spiral at a fixed left-center origin.
      ...
      // Apply transition and smooth movement for each image
    ...
    // Set the camera target to the world coordinates of the group
    targetOriginX = CAMERA_CENTER_X - LEFT_CENTER_X * scale;
