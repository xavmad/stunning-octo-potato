...
function activateGroup(project) {
  ...
  /* zoom out from fixed center of the screen (whole canvas) */
  targetScale = 0.7;
  ...
  setTimeout(() => {
    ...
    /* ==========================================
       WORLD SPACE — FIXED CAMERA CENTER
    ========================================== */

    // Zoom out from center of the screen (viewport)
    const worldCenterX = (window.innerWidth * 0.5 - originX) / scale;
    const worldCenterY = (window.innerHeight * 0.5 - originY) / scale;

    // Fix group to the left-center of the screen
    const CENTER_X = worldCenterX - (window.innerWidth * 0.35) / scale;
    ...
    /* ==========================================
       CAMERA FRAMES GROUP — Fixed from center-left
    ========================================== */

    targetScale = 1;
    // Instead of using originX and originY, set camera origin to a fixed point
    targetOriginX = CAMERA_CENTER_X - CENTER_X;
    targetOriginY = CAMERA_CENTER_Y - CENTER_Y;

    // Smooth zoom-in transition from the fixed point
