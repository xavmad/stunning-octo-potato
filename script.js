...
function activateGroup(project) {
  ...
  // Prevent the camera from re-framing during zoom transition
  if (isZooming) return;

  isZooming = true; // Set flag to true to indicate zooming is in progress
  ...
  setTimeout(() => {
    ...
    // Fixed starting position: Set the group to be at the exact left-center of the screen
    const LEFT_CENTER_X = window.innerWidth * 0.1;  // Fixed 10% from the left of the screen
    const CENTER_Y = window.innerHeight * 0.5; // Fixed center vertically
    ...
    groupImages.forEach((img, i) => {
      ...
      // Golden-angle spiral for the layout (preserving the original spiral behavior)
      const angle = i * Math.PI * (3 - Math.sqrt(5)); // Golden angle
      const radius = Math.sqrt(i) * (IMAGE_SIZE + MIN_GAP);

      // **Key Fix:** The first image (center image) starts at the left-center
      let x = LEFT_CENTER_X + Math.cos(angle) * radius;
      let y = CENTER_Y + Math.sin(angle) * radius;

      // Ensure images are within screen bounds
    ...
    // Ensure the camera always repositions when zooming in
    // Adjust camera position dynamically based on the zoom scale
    targetScale = 1;
    targetOriginX = CAMERA_CENTER_X - LEFT_CENTER_X;
    targetOriginY = CAMERA_CENTER_Y - CENTER_Y;

    // Set the camera to the left-center position and avoid moving after zoom
    setTimeout(() => {
      // Prevent any unintended camera shifts after zooming
      targetOriginX = CAMERA_CENTER_X - LEFT_CENTER_X;
      targetOriginY = CAMERA_CENTER_Y - CENTER_Y;
  ...
  // Reset flag after the zoom-in transition
  setTimeout(() => {
    isZooming = false;
  }, 800); // Ensure the zoom-in transition time is completed before resetting
}
