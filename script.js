...
images.forEach(img => {
  // Initialize random positions for images on page load
  ...
  // Assign random velocities for floating motion
  img._vx = (Math.random() - 0.5) * 0.12;
  img._vy = (Math.random() - 0.5) * 0.12;

  img._floating = true;

  // Apply the initial transform for floating images
  img.style.transform =
...
function floatImages() {
  if (!activeProject || IS_MOBILE) {
    images.forEach(img => {
      ...
      // Apply floating motion after initial setup
      img._tx += img._vx;
      img._ty += img._vy;
      ...
      // Apply the floating motion with smooth transformation
      img.style.transform =
...
/* =====================================================
   GROUP LOGIC — LEFT SIDE ADAPTIVE SPIRAL (CORRECTED)
===================================================== */
...
function activateGroup(project) {
  ...
  setTimeout(() => {
    ...
    // actual screen center in WORLD space (zoom-safe)
    const worldCenterX =
    ...
    const CENTER_X =
      worldCenterX - (window.innerWidth * 0.25) / scale;
