...
function activateGroup(project) {
  ...
  // Adjust opacity of images in the selected group
  images.forEach(img => {
    img.style.opacity =
  ...
  /* Zoom out slightly like the original behavior */
  targetScale = 0.7;
  ...
  setTimeout(() => {
    ...
    // World space center (adjusted for zooming)
    ...
    // Calculate position for each image in the group
    ...
    // Fix camera position to the left-center of the screen
    targetScale = 1;

    // Adjust target origin to always center-left on the canvas
