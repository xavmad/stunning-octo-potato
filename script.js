...
function activateGroup(project) {
  ...
  setTimeout(() => {
    ...
    /* =====================================================
       DYNAMIC SPACING BASED ON IMAGE COUNT
    ===================================================== */

    /*
      few images  → large spacing
      many images → tight spacing
    */

    const density = Math.min(Math.max(COUNT / 14, 0.6), 2.2);

    const ANGLE_STEP  = 0.9;
    const BASE_RADIUS = IMAGE_SIZE * 0.9;
    ...
    groupImages.forEach((img, i) => {
      ...
      const radius = Math.min(i * RADIUS_STEP, MAX_RADIUS);
