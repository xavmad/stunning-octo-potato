...
function activateGroup(project) {
  ...
  setTimeout(() => {
    ...
    // Fixed starting position: place the group at center-left of the screen
    ...
    /* adaptive radius */
    const MAX_RADIUS = Math.min(window.innerWidth * 0.28, window.innerHeight * 0.45);
    ...
    groupImages.forEach((img, i) => {
      ...
      const radius = Math.min(spiralRadius * 0.4 + linearRadius * 0.6, MAX_RADIUS);
