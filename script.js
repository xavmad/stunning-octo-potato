...
/* =====================================================
   MOUSE POSITION TRACKING (DESKTOP ZOOM)
===================================================== */
...
function cameraLoop() {
 ...
 if (Math.abs(zoomVelocity) > 0.00001) {
  ...
  const zoom = Math.exp(zoomVelocity);
...
viewport.addEventListener("wheel", e => {
  ...
  // normalize wheel vs trackpad
  delta = Math.max(-120, Math.min(120, delta));
  delta = delta / 120;

  zoomVelocity += -delta * 0.035;

  // ✅ STEP 4 — CLAMP VELOCITY HERE
