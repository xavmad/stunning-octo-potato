...
function activateGroup(project) {
  ...
  setTimeout(() => {
    ...
    // Adjust CENTER_X to align with the left side of the screen (not the whole screen center)
    const CENTER_X =
      worldCenterX - (window.innerWidth * 0.35) / scale;  // Adjusted to shift it further left
    ...
    targetScale = 1;
    targetOriginX = CAMERA_CENTER_X - CENTER_X * scale;  // Fix CAMERA ORIGIN based on WORLD SPACE
    targetOriginY = CAMERA_CENTER_Y - CENTER_Y * scale;  // Fix CAMERA ORIGIN based on WORLD SPACE
