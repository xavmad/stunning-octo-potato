...
function activateGroup(project) {
  ...
  setTimeout(() => {
    ...
    const density =
      Math.min(Math.max(COUNT / 14, 0.6), 2.2);

    const ANGLE_STEP  = 0.9;
    const BASE_RADIUS = IMAGE_SIZE * 0.9;
    const RADIUS_STEP = BASE_RADIUS / density;
    ...
    groupImages.forEach((img, i) => {
      ...
      const ringIndex = Math.ceil(i / 2);

      const angle =
        direction * ringIndex * ANGLE_STEP;

      const radius =
        Math.min(ringIndex * RADIUS_STEP, MAX_RADIUS);
