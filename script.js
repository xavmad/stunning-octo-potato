...
/* =====================================================
   GROUP LOGIC — LEFT-ANCHOR SPIRAL
===================================================== */
...
function activateGroup(project) {
  ...
  // zoom out slightly for animation
  targetScale = 0.7;
  targetOriginX = 0;
  targetOriginY = 0;
  ...
  setTimeout(() => {
    ...
    /* ==========================================
       SCREEN-SAFE WORLD BOUNDS
    ========================================== */

    const LEFT   = -window.innerWidth  / 2 + HALF;
    const RIGHT  =  window.innerWidth  / 2 - HALF;
    const TOP    = -window.innerHeight / 2 + HALF;
    const BOTTOM =  window.innerHeight / 2 - HALF;

    /* ==========================================
       ANCHOR IMAGE (FIRST IMAGE)
    ========================================== */

    const ANCHOR_X = -window.innerWidth * 0.25;
    const ANCHOR_Y = 0;

    const placed = [];

    /* ---- place first image exactly ---- */

    const first = groupImages[0];

    first.style.width = IMAGE_SIZE + "px";
    ...
    first.style.transform =
      `translate(${ANCHOR_X}px, ${ANCHOR_Y}px) translate(-50%, -50%)`;

    placed.push({ x: ANCHOR_X, y: ANCHOR_Y });

    /* ==========================================
       SPIRAL REMAINING IMAGES
    ========================================== */
    ...
    groupImages.slice(1).forEach((img, i) => {

      img.style.width = IMAGE_SIZE + "px";

      let radius = Math.min(
        Math.sqrt(i + 1) * (IMAGE_SIZE + 80),
        MAX_RADIUS
      );
      ...
      do {
        ...
        angle += 0.35;
        radius = Math.min(radius + 12, MAX_RADIUS);
        tries++;
      ...
      img.style.zIndex = 1000 + i + 1;
      img.style.transform =
        `translate(${x}px, ${y}px) translate(-50%, -50%)`;
    });

    /* focus camera on anchor */
    targetScale = 1;
    targetOriginX = -ANCHOR_X;
    targetOriginY = -ANCHOR_Y;
