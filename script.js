...
function activateGroup(project) {
  ...
  /* Zoom out slightly like the original */
  targetScale = 0.7;
  ...
  setTimeout(() => {
    ...
    // Fixed positioning: Ensure group is at center-left of the screen
    const CENTER_X = window.innerWidth * 0.1;  // Left-center position
    const CENTER_Y = window.innerHeight * 0.5; // Vertical center
    ...
    groupImages.forEach((img, i) => {
      ...
      // Calculate x and y coordinates with spiral effect
      let x = CENTER_X + Math.cos(angle) * radius;
      let y = CENTER_Y + Math.sin(angle) * radius;

      // Keep images within bounds (ensuring no overflow off-screen)
      ...
      // Animation handling: Ensure images stack with correct z-index
