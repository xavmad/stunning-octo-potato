...
function activateGroup(project) {
  // Prevent camera actions if zoom transition is still ongoing
  if (isZooming) {
    // End the zoom transition and force the camera to its final state immediately
    scale = 1;  // Immediately reset zoom scale to 1
    targetScale = 1;
  ...
  setTimeout(() => {
    ...
    // Immediately stop the zoom and reframe the camera to left-center
    setTimeout(() => {
      targetScale = 1;
      targetOriginX = CAMERA_CENTER_X - LEFT_CENTER_X;
      targetOriginY = CAMERA_CENTER_Y - CENTER_Y;
