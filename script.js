...
const mobileView = document.getElementById("mobile-project-view");
...
images.forEach(img => {
 img.addEventListener("click", e => {
  ...
  if (IS_MOBILE) {
    openMobileProject(img.dataset.project);
  } else {
    if (activeProject) return;
...
function openMobileProject(project) {
  ...
  // first image
  mobileContent.appendChild(groupImages[0].cloneNode());

  // description
  if (description) {
    mobileContent.appendChild(description.cloneNode(true));
  }

  // remaining images
  groupImages.slice(1).forEach(img => {
    mobileContent.appendChild(img.cloneNode());
  });
