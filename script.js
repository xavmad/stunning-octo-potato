...
function clampPan() {
  ...
  const limitX =
    window.innerWidth * (scale - 1) + PAN_PADDING;

  const limitY =
    window.innerHeight * (scale - 1) + PAN_PADDING;

  targetOriginX = applyEdgeResistance(
    targetOriginX,
    limitX
  );

  targetOriginY = applyEdgeResistance(
    targetOriginY,
    limitY
  );
}
...
function applyEdgeResistance(value, limit, resistance = 0.12) {
  if (value > limit) {
    return limit + (value - limit) * resistance;
  }
  if (value < -limit) {
    return -limit + (value + limit) * resistance;
  }
