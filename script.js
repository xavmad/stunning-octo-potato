...
function activateGroup(project) {
  ...
  setTimeout(() => {
    ...
    const screenCenterWorldX = -targetOriginX / targetScale;
    const screenCenterWorldY = -targetOriginY / targetScale;

    /* LEFT-CENTER OF SCREEN */
    const CENTER_X =
      screenCenterWorldX -
      (window.innerWidth * 0.25) / targetScale;

    const CENTER_Y = screenCenterWorldY;

    /* keep spiral inside viewport */
    const MAX_RADIUS =
      Math.min(
    ...
    targetScale = 1;
    targetOriginX = -CENTER_X;
    targetOriginY = -CENTER_Y;
