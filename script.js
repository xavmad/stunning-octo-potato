...
// EXPAND VIRTUAL WORLD ON MOBILE
const WORLD_SCALE = IS_MOBILE ? 2.5 : 1;

// OPTIONAL — more zoom freedom on mobile
const MOBILE_MIN_SCALE = 0.5;
const MOBILE_MAX_SCALE = 3.2;
...
const MIN_SCALE = IS_MOBILE ? MOBILE_MIN_SCALE : 0.6;
const MAX_SCALE = IS_MOBILE ? MOBILE_MAX_SCALE : 1.8;
...
images.forEach(img => {
  const x = Math.random() * window.innerWidth  * WORLD_SCALE;
  const y = Math.random() * window.innerHeight * WORLD_SCALE;
...
function floatImages() {
  ...
  if (!activeProject || IS_MOBILE) {   // ✅ ADDED MOBILE GUARD
    ...
    images.forEach(img => {
      ...
      const m = 80;
      const maxX = window.innerWidth  * WORLD_SCALE - m;
      const maxY = window.innerHeight * WORLD_SCALE - m;
...
viewport.addEventListener("touchmove", e => {
  ...
  if (e.touches.length === 2) {
    ...
    if (lastTouchDist) {
      ...
      const worldX = (midX - originX) / scale;
      const worldY = (midY - originY) / scale;
