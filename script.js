...
function activateGroup(project) {
  ...
  setTimeout(() => {
    ...
    // Get the world coordinates for the center (considering zoom)
    const worldCenterX =
      (window.innerWidth * 0.5 - originX) / scale;
    const worldCenterY =
      (window.innerHeight * 0.5 - originY) / scale;

    // Adjust to the left-center of the screen in world space
    const CENTER_X = worldCenterX - (window.innerWidth * 0.25) / scale;
    ...
    // Adjusting the camera origin to focus on the group
    targetScale = 1;
    targetOriginX = CAMERA_CENTER_X - CENTER_X; // FIX CAMERA ORIGIN
    targetOriginY = CAMERA_CENTER_Y - CENTER_Y; // FIX CAMERA ORIGIN

    // Apply a smoother zoom effect when entering the group
    scale += (targetScale - scale) * 0.1;

    // Allow time for the transition before showing descriptions
    setTimeout(() => {
  ...
  }, 520); // Keep a delay to ensure the images are placed before transitioning
}
