...
function activateGroup(project) {
  ...
  targetScale = 0.7;
  scale = 0.7;  // Force the scale immediately to match the target scale

  // Force position based on the zoom level
  ...
  setTimeout(() => {
    ...
    const CENTER_X = -window.innerWidth * 0.33 / layoutScale;
    const CENTER_Y = 0;
    ...
    // Lock target scale to 1 after the group animation
    targetScale = 1;
    targetOriginX = CAMERA_CENTER_X - CENTER_X;
    targetOriginY = CAMERA_CENTER_Y - CENTER_Y;

    // Apply a smoother zoom effect
    scale += (targetScale - scale) * 0.1;  // Smooth transition for zoom
